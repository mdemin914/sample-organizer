...
const openaiUtil_1 = require("../openaiUtil");
...
describe("mapFilenamesToFolders (live)", () => {
    it("returns a mapping from the real OpenAI API", async () => {
        ...
        const result = await (0, openaiUtil_1.mapFilenamesToFolders)({
    ...
    it("classifies Cymatics melody sample correctly", async () => {
        ...
        const file = "Cymatics - 2022 Melody Collection (BETA)/2022 Melody Collection (BETA)/DrillCymatics - Empty The Clip - 140 BPM D Min.wav";
        const res = await (0, openaiUtil_1.classifyFile)(file, {
            folders,
            apiKey: process.env.OPENAI_API_KEY,
        });
...
//# sourceMappingURL=openaiUtil.test.js.map